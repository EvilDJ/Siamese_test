...
class SPPLayer():
    def __init__(self, bins, feature_map_size):
        ...
        self.a = float(feature_map_size)  # 14
        self.bins = bins  # bins = [3, 2, 1]
        self.n = len(bins)
    
    def spatial_pyramid_pooling(self, data):
        self.input = data  # <tf.Tensor 'Processing/conv5_3/Relu:0' shape=(32, 14, 14, 512) dtype=float32>
        self.batch_size = self.input.get_shape().as_list()[0]  # 32
