...
def get_pic(image_flie_l,image_flie_r,num):
    ...
    image_l = sc.robin(image_l)
    img_l = cv2.resize(image_l, (225, 225))
    img_l = np.array(img_l)
    img_l = re_shape(img_l)
    dictionary_r = {}
    ...
    image_r = sc.robin(image_r)
    img_r = cv2.resize(image_r, (225, 225))
    # img_r.append(image_r)
    img_r = re_shape(img_r)
    return img_l,img_r
