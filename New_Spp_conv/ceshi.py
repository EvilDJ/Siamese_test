...
tes_tra_l = conv_net.siamese(xl_test)
tes_tra_r = conv_net.siamese(xr_test)
tes_tra_loss, tes_tra_distace = conv_net.loss(tes_tra_l, tes_tra_r, label)
...
with tf.Session() as sess:
    ...
    for train, test in kf.split(separate_image):
        ...
        for num_tes in test:
            ...
            tes_tes_distance_s = sess.run(tes_tra_distace, feed_dict={xl_test: pic_tes_l_s, xr_test: pic_tes_r_s,
                                                                      label: int(list_[num_tes][0])})
...
input_1 = tf.placeholder(tf.float32,shape=(1,225,225,3))