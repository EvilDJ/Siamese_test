...
"""Inception model from http://arxiv.org/abs/1512.00567.
  ...
  Args:
    ...
    final_endpoint: specifies the endpoint to construct the network up to. 指定要构造的网络端点It
  ...
  Returns:
    tensor_out: output tensor corresponding to the final_endpoint.与final_endpoint对应的输出张量
    end_points: a set of activations for external use, for example summaries or
                losses.一组用于外部激活的，例如summaries 和loss值
...
"""Inception model from http://arxiv.org/abs/1512.00567.
  ...
  depth_multiplier.
  使用默认参数，该方法构造了在其中定义的精确模型这张纸上。但是，可以尝试使用inception_v3的变体通过改变参数dropout_keep_prob、min_depth和depth_multiplier
  ...
  Args:参数
    ...
    dropout_keep_prob: the percentage of activation values that are retained.保留的激活值的百分比
    min_depth: Minimum depth value (number of channels) for all convolution ops.
      Enforced when depth_multiplier < 1, and not an active constraint when
      depth_multiplier >= 1.
    depth_multiplier: Float multiplier for the depth (number of channels)
      ...
      usage will be to set this value in (0, 1) to reduce the number of
      parameters or computation cost of the model.
    prediction_fn: a function to get predictions out of logits.从逻辑中得到预测的函数
    spatial_squeeze: if True, logits is of shape [B, C], if false logits is
        of shape [B, 1, 1, C], where B is batch_size and C is number of classes.
    reuse: whether or not the network and its variables should be reused. To be
      able to reuse 'scope' must be given.网络及其变量是否应该重用。是必须赋予重用“范围”的能力
    scope: Optional variable_scope.
...
def inception_v3(inputs, num_classes=1000, is_training=True, dropout_keep_prob=0.8, min_depth=16,
                 depth_multiplier=1.0, prediction_fn=slim.softmax, spatial_squeeze=True, reuse=None,scope='InceptionV3'):
  if depth_multiplier <= 0:
    raise ValueError('depth_multiplier is not greater than zero.')
  depth = lambda d: max(int(d * depth_multiplier), min_depth)
  with tf.variable_scope(scope, 'InceptionV3', [inputs, num_classes], reuse=reuse) as scope:
    with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
      ...
      # Auxiliary Head logits 先把卷积、最大池化、平均池化默认步长设为1
      with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
        aux_logits = end_points['Mixed_6e']
        with tf.variable_scope('AuxLogits'):
            # Shape of feature map before the final layer.
            #把6e之后连接的一个5*5平均池化，步长为3，padding为VALID，
            # 这样的尺寸从17*17*68变成5*5*768
...
"""Define kernel size which is automatically reduced for small input.
  ...
  如果输入图像的形状在图的构造时是未知的 函数假设输入的图像足够大
...
让这个函数在未知形状下工作。从理论上讲,这可以使用下面的代码
  """
...
inception_v3_arg_scope = inception_untils.inception_arg_scope
